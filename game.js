(() => {
  ...
  function loadCourse(number) {
    ...
    world.stageModifiers=(window.BETA_STAGE_MODIFIERS || {})[number] || {};
  ...
  function resetGame() { world.camera=0; world.backgroundOffset=0; world.time=0; world.checkpointIndex=0; world.completed=0; world.complete=0; world.repaired=0; world.combo=0; world.comboTimer=0; world.repairCombo=0; world.repairComboTimer=0; world.recoveryGlow=0; world.groundPound=0; world.hitStop=0; world.temporaryPlatforms=[]; world.footsteps=[]; world.history=[]; world.thrownOrbs=[]; world.bellWaves=[]; world.guideEchoes=[]; world.gravityDirection=1; world.timeShifted=false; world.todoTimer=0; world.lastGround=null; world.stats={orbs:0,repairs:0,enemies:0,checkpoints:0,jumps:0,dashes:0,attacks:0}; world.stageTipShown=false; world.messageShown=false; world.gateHintShown=false; world.guideSeen=false; world.particles=[]; world.afterimages=[]; world.repairWaves=[]; world.attackFlash=0; world.gateExit=0; world.gateExitParticles=[]; world.stageClear=false; world.courseSelect=false; world.floating=false; setControlGuide(false); $('#devFloat').textContent='浮遊：OFF'; $('#stageClear').hidden=true; $('#courseSelect').hidden=true; const start=checkpoints[0],startPlatform=platforms.find(p=>p.id===start.platformId); player.x=start.x; player.y=(startPlatform?.y||520)-20-player.h; player.vx=0; player.vy=0; player.attack=0; player.attackCooldown=0; player.groundDash=0; player.dashCooldown=0; player.airDash=0; player.airDashAvailable=abilities.airDash; player.doubleJumpAvailable=abilities.doubleJump; player.coyote=0; player.jumpBuffer=0; player.orbCharges=0; player.scaffoldCharges=0; player.checkpointGuard=0; player.lastFootstepCell=null; player.charging=0; player.chargeReady=false; player.invulnerable=0; shards.forEach(s=>s.taken=false); repairPoints.forEach(p=>{p.repaired=false;p.promptShown=false;}); platforms.forEach(p=>p.active=p.defaultActive ?? !['bridge-a','bridge-b'].includes(p.id)); checkpoints.forEach((p,i)=>p.active=i===0); enemies.forEach(e=>{e.alive=true;e.purified=false;e.stun=0;}); $('#completeBar').style.width='0%'; updateHud(); $('#runTimer').textContent='00:00.00'; setMenu(false); showDialogue(abilities.airDash?'ピース「Air Dashが使えるよ！ 空中で X を押して、向いている方向へ飛ぼう。」':GAME_CONFIG.initialDialogue); }
  ...
  addEventListener('keydown', (e) => {
    ...
    if (!e.repeat && world.started && !world.menuOpen && !world.courseSelect && !world.stageClear) {
      if (e.code==='KeyB' && FEATURES.checkpointRecall) { e.preventDefault(); respawn(); showToast('ピース「最後のチェックポイントへ戻った。」'); return; }
      if (e.code==='KeyF' && FEATURES.groundPound && !player.grounded) { e.preventDefault(); world.groundPound=.45; player.vy=1050; showToast('ピース「急降下！」'); return; }
      if (e.code==='KeyG' && FEATURES.chargeAttack && player.attackCooldown<=0) { e.preventDefault(); strike(2); world.attackFlash=.34; showToast('ピース「ため攻撃！」'); return; }
  ...
  addEventListener('keyup', (e) => { keys.delete(e.code); if (FEATURES.variableJump && e.code===bindings.jump && player.vy < -220) player.vy *= .52; if(FEATURES.chargeAttack && e.code===bindings.attack && player.charging>.28){player.chargeReady=true; player.charging=0; showToast('ピース「ため攻撃の準備完了！」');}else if(e.code===bindings.attack)player.charging=0; });
  ...
  function leavePurificationGuide(x,y) {
    if(!FEATURES.purificationGuideTrail) return;
    const target=repairPoints.find(point=>!point.repaired) || shards.find(shard=>!shard.taken) || {x:goalGate.x,y:goalGate.y-72};
  ...
  function throwOrb() {
    const scaffoldReady=FEATURES.checkpointScaffoldCharge && player.scaffoldCharges>0;
    if (!FEATURES.orbThrow || (player.orbCharges<=0 && !scaffoldReady)) return;
    const usesScaffold=player.orbCharges<=0;
    if(usesScaffold) player.scaffoldCharges--; else player.orbCharges--;
    world.thrownOrbs.push({x:player.x+player.w/2,y:player.y+28,vx:player.facing*mechanic('orbThrowSpeed'),vy:-90,life:mechanic('orbThrowLife'),age:0,buildable:FEATURES.orbBuildPlatform||usesScaffold});
    showToast(usesScaffold?'ピース「チェックポイントの設計図を足場にした！」':FEATURES.orbBuildPlatform?'ピース「オーブを足場に変える！」':'ピース「オーブを投げた！」');
  }
  ...
  function strike(power=1) {
    ...
    const reach=power>1?164:98; const hitbox = { x: player.facing > 0 ? player.x + player.w - 3 : player.x-reach+2, y: player.y + 10, w: reach, h: 54 };
    ...
    if(FEATURES.attackOrbBurst && player.orbCharges>0)world.repairWaves.push({x:player.x+player.w/2,y:player.y+34,life:.26,color:'#8ff7ff',max:54});
  }
  ...
  function update(dt) {
    ...
    else if (!world.floating && FEATURES.doubleJump && abilities.doubleJump && player.jumpBuffer>0 && player.doubleJumpAvailable) { player.vy=-tuning.jumpVelocity*.9; player.doubleJumpAvailable=false; player.jumpBuffer=0; world.stats.jumps++; world.repairWaves.push({x:player.x+player.w/2,y:player.y+player.h/2,life:.32,color:'#cba8ff',max:46}); keys.delete(bindings.jump); keys.delete('jump'); }
    ...
    for (const enemy of enemies) {
      ...
      enemy.stun=Math.max(0,(enemy.stun||0)-dt); if(FEATURES.enemyStun && enemy.stun>0) continue;
      const beat=FEATURES.rhythmPatrol ? .55+Math.abs(Math.sin(world.time*mechanic('rhythmPlatformSpeed')+enemy.phase))*.9 : 1;
      enemy.x += enemy.speed * enemy.dir * dt * beat * (world.timeShifted || (FEATURES.comboTimeSlow && world.combo>=3) ? mechanic('timeShiftEnemyRate') : 1);
      ...
      const stomp=FEATURES.enemyBounce && player.vy>180 && player.y+player.h-player.vy*dt<=enemy.y+15 && player.x+player.w>enemy.x+5 && player.x<enemy.x+enemy.w-5;
      if(stomp){player.y=enemy.y-player.h;player.vy=-mechanic('enemyBounceVelocity');player.grounded=false;player.airDashAvailable=abilities.airDash;enemy.stun=mechanic('enemyBounceStun');world.repairWaves.push({x:enemy.x+enemy.w/2,y:enemy.y+8,life:.28,color:'#b8ffdf',max:50});if(FEATURES.particles)for(let i=0;i<10;i++)world.particles.push({x:enemy.x+enemy.w/2,y:enemy.y+8,vx:(Math.random()-.5)*165,vy:-60-Math.random()*145,life:.48,color:'#b8ffdf'});showToast('ピース「敵を踏み台にして跳ねた！」');}
      else if (!player.invulnerable && rect(player, { x: enemy.x, y: enemy.y, w: enemy.w, h: enemy.h })) {
        if(FEATURES.checkpointGuardSeal && player.checkpointGuard>0){player.checkpointGuard--;player.invulnerable=mechanic('checkpointGuardDuration');player.vx=-enemy.dir*260;player.vy=-260;enemy.stun=1.4;world.repairWaves.push({x:player.x+player.w/2,y:player.y+34,life:.32,color:'#bdf5ff',max:62});showToast('ピース「チェックポイントの守りが攻撃を防いだ！」');}
        else if(FEATURES.orbShield && player.orbCharges>0){player.orbCharges--;player.invulnerable=1;showToast('ピース「オーブシールドが守ってくれた！」');}
        else respawn('fall');
      }
    }
    if (FEATURES.collectibles) for (const shard of shards) if (!shard.taken) {
      if (FEATURES.orbMagnet) {
        ...
        const magnetRadius=FEATURES.dashOrbAttract && player.groundDash>0 ? 260 : 145;
        if (distance<magnetRadius && distance>1) { shard.x+=dx/distance*dt*(player.groundDash>0?430:210); shard.y+=dy/distance*dt*(player.groundDash>0?430:210); }
      }
    ...
    for (let i=world.checkpointIndex+1;i<checkpoints.length;i++) {
      ...
      if (player.x >= point.x) {
        const running=Math.abs(player.vx)>=mechanic('checkpointMomentumThreshold');
        world.checkpointIndex=i; point.active=true; world.stats.checkpoints++;
        const rewards=[];
        if(FEATURES.checkpointScaffoldCharge){player.scaffoldCharges=Math.max(player.scaffoldCharges,mechanic('checkpointScaffoldCharges'));rewards.push('設計図 ×1');}
        if(FEATURES.checkpointGuardSeal){player.checkpointGuard=1;player.invulnerable=Math.max(player.invulnerable,mechanic('checkpointGuardGrace'));rewards.push('守りの印');}
        if(FEATURES.checkpointMomentumBoost && running){player.checkpointMomentum=mechanic('checkpointMomentumDuration');rewards.unshift('風をつないだ');}
        showToast(`ピース「チェックポイント更新！${rewards.length?` ${rewards.join('・')} を受け取った。`:''}」`);
    ...
    if (FEATURES.courseTips && FEATURES.stageFlowHints && !world.stageTipShown && world.time>.12) { world.stageTipShown=true; showToast(`ピース「${world.stageDesign?.learn || '青い風標が次の足場への道しるべだよ。'}」`); }
    updateNarrative(dt);
  ...
  function draw() {
    ...
    if(FEATURES.orbRestorationTint && world.colorRecovery>0){ctx.save();ctx.globalCompositeOperation='screen';ctx.globalAlpha=world.colorRecovery*.16;ctx.fillStyle='#79e9ff';ctx.fillRect(0,0,GAME_CONFIG.width,GAME_CONFIG.height);ctx.restore();}
    ...
    if(FEATURES.completionCloudBloom && world.colorRecovery>0){ctx.save();ctx.globalAlpha=world.colorRecovery*.24;ctx.fillStyle='#e7fbff';for(let i=0;i<5;i++){const x=110+i*285,y=548+(i%2)*38;ctx.beginPath();ctx.ellipse(x,y,62,14,0,0,Math.PI*2);ctx.ellipse(x+42,y-8,48,16,0,0,Math.PI*2);ctx.fill();}ctx.restore();}
    ...
    const restoredCount=repairPoints.filter(point=>point.repaired).length;
    if(FEATURES.orbMemorySense && narrative && !world.fragmentTaken && player.orbCharges>0){const dx=narrative.fragment[0]-(player.x+player.w/2),dy=narrative.fragment[1]-(player.y+player.h/2),distance=Math.hypot(dx,dy);if(distance<mechanic('memorySenseDistance')){ctx.save();ctx.translate(player.x+player.w/2,player.y+player.h/2);ctx.globalAlpha=Math.max(.18,1-distance/mechanic('memorySenseDistance'));ctx.strokeStyle='#d7b5ff';ctx.shadowColor='#d3a8ff';ctx.shadowBlur=12;ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,30+Math.sin(world.time*6)*5,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.moveTo(dx*.15,dy*.15);ctx.lineTo(dx*.15-7,dy*.15+7);ctx.moveTo(dx*.15,dy*.15);ctx.lineTo(dx*.15+7,dy*.15+7);ctx.stroke();ctx.restore();}}
    if(narrative && FEATURES.futureWindows && images.futureWindow.complete){ctx.save();ctx.globalAlpha=FEATURES.futureWindowRestoration?Math.min(1,.58+restoredCount*.28):1;ctx.drawImage(images.futureWindow,narrative.window[0]-88,narrative.window[1]-145,176,220);if(FEATURES.futureWindowRestoration && restoredCount){ctx.globalCompositeOperation='screen';ctx.globalAlpha=.12*restoredCount;ctx.fillStyle='#ffeaa2';ctx.fillRect(narrative.window[0]-75,narrative.window[1]-126,150,166);}ctx.restore();}
    ...
    if(narrative && FEATURES.restoredResidents && restoredCount && images.restoredResident.complete){const anchor=repairPoints[0];const residentX=anchor.x+92,residentY=anchor.y-20+Math.sin(performance.now()/280)*4;ctx.save();ctx.translate(residentX,residentY);ctx.shadowColor='#a8fff0';ctx.shadowBlur=17;ctx.drawImage(images.restoredResident,-30,-38,60,72);ctx.restore();if(FEATURES.residentLanternTrail){const target=shards.find(shard=>!shard.taken) || {x:goalGate.x,y:goalGate.y-72};ctx.save();ctx.fillStyle='#fff0a3';ctx.shadowColor='#8ff6ff';ctx.shadowBlur=12;for(let i=1;i<=4;i++){const t=i/5;ctx.globalAlpha=.72-i*.1;ctx.fillRect(residentX+(target.x-residentX)*t-3,residentY+(target.y-residentY)*t-3,6,6);}ctx.restore();}}
    if(narrative && FEATURES.dynamicSigns){const signComplete=FEATURES.repairSignCompletion && restoredCount>0;const signText=signComplete?`DONE: ${world.stageDesign?.payoff || '修復済み'}`:narrative.sign;ctx.save();ctx.fillStyle=signComplete?'#143626df':'#07142ad9';ctx.strokeStyle=signComplete?'#a9f7a2':'#7ccdf4';ctx.lineWidth=2;ctx.fillRect(narrative.window[0]-85,narrative.window[1]+75,170,28);ctx.strokeRect(narrative.window[0]-85,narrative.window[1]+75,170,28);ctx.fillStyle=signComplete?'#e7ffd8':'#e2f7ff';ctx.font='bold 9px sans-serif';ctx.textAlign='center';ctx.fillText(signText,narrative.window[0],narrative.window[1]+93);ctx.textAlign='start';ctx.restore();}
    ...
    for (const point of repairPoints) {
      ...
      if(FEATURES.repairAmbientFireflies && point.repaired){ctx.save();ctx.fillStyle='#fff4ae';ctx.shadowColor='#72efff';ctx.shadowBlur=10;for(let i=0;i<4;i++){const a=world.time*2+i*1.57;ctx.globalAlpha=.55+Math.sin(a)*.25;ctx.fillRect(point.x+Math.cos(a)*42-2,point.y-54+Math.sin(a*1.3)*22-2,4,4);}ctx.restore();}
      const bridge=platforms.find(platform=>platform.id===point.platformId);
      if(FEATURES.repairOutcomePreview && !point.repaired && bridge && Math.hypot(player.x+player.w/2-point.x,player.y+player.h/2-point.y)<mechanic('repairPreviewDistance')){
        const targetX=platformX(bridge)+bridge.w/2,targetY=platformY(bridge)-26;
        ctx.save();ctx.globalAlpha=.6;ctx.strokeStyle='#fff0a1';ctx.shadowColor='#72eaff';ctx.shadowBlur=11;ctx.lineWidth=2;ctx.setLineDash([7,7]);ctx.lineDashOffset=-world.time*24;ctx.beginPath();ctx.moveTo(point.x,point.y-22);ctx.quadraticCurveTo((point.x+targetX)/2,Math.min(point.y,targetY)-82,targetX,targetY);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='#eaffff';ctx.font='bold 9px sans-serif';ctx.textAlign='center';ctx.fillText('修復後の道',targetX,targetY-12);ctx.textAlign='start';ctx.restore();
      }
    ...
    if(FEATURES.repairAssembly) for(const build of world.repairBuilds) {
      ...
      ctx.save();ctx.globalAlpha=(1-progress)*.9;ctx.strokeStyle='#fff1a4';ctx.shadowColor='#75eaff';ctx.shadowBlur=16;ctx.lineWidth=4;
      ctx.setLineDash([10,8]);ctx.lineDashOffset=-progress*48;ctx.beginPath();ctx.moveTo(build.x,build.y-18);ctx.lineTo(build.targetX,build.targetY);ctx.stroke();ctx.setLineDash([]);
      for(let i=0;i<6;i++){const t=Math.max(0,progress-i*.09);ctx.fillStyle=i%2?'#f8eeb2':'#75eaff';ctx.fillRect(build.x+(build.targetX-build.x)*t-4,build.y-18+(build.targetY-(build.y-18))*t-4,8,8);}ctx.restore();
    }
    ...
    if(FEATURES.orbGateFlight) for(const flight of world.orbFlights) {
      const t=1-flight.life/flight.max, eased=t*t*(3-2*t); const x=flight.x+(flight.targetX-flight.x)*eased; const y=flight.y+(flight.targetY-flight.y)*eased-Math.sin(t*Math.PI)*72;
      ctx.save();ctx.translate(x,y);ctx.rotate(t*Math.PI*3);ctx.globalAlpha=.7+.3*(1-t);ctx.fillStyle='#eaffff';ctx.shadowColor='#62ebff';ctx.shadowBlur=22;ctx.fillRect(-7,-7,14,14);ctx.fillStyle='#fff2a0';ctx.fillRect(-3,-3,6,6);ctx.restore();
    }
    ...
    if(world.gateExit > 0) { const dissolve=Math.min(1,(.92-world.gateExit)/.58); ctx.save(); ctx.beginPath(); ctx.rect(-80,-18,160,Math.max(0,124*(1-dissolve))); ctx.clip(); drawPlayerSprite(); ctx.restore(); } else drawPlayerSprite(); if(FEATURES.orbEmbroidery && player.orbCharges>0){const stitches=Math.min(6,player.orbCharges);ctx.save();ctx.fillStyle='#c8ffff';ctx.shadowColor='#5deaff';ctx.shadowBlur=8;for(let i=0;i<stitches;i++)ctx.fillRect(-20+i*7,31+(i%2)*5,3,3);ctx.restore();} if(FEATURES.attackArc&&world.attackFlash>0){ctx.globalAlpha=world.attackFlash/.16;ctx.strokeStyle='#bdf9ff';ctx.lineWidth=5;ctx.beginPath();ctx.arc(player.facing*24,34,48,-1.1,1.1);ctx.stroke();}ctx.restore();
    ...
    if(FEATURES.enemyRouteGhosts && Math.abs(player.vx)<12) for(const enemy of enemies)if(enemy.alive){ctx.save();ctx.globalAlpha=.22;ctx.strokeStyle='#dcaaff';ctx.setLineDash([5,7]);ctx.beginPath();ctx.moveTo(enemy.min,enemy.y+enemy.h+3);ctx.lineTo(enemy.max+enemy.w,enemy.y+enemy.h+3);ctx.stroke();ctx.setLineDash([]);ctx.restore();}
    ...
    if(FEATURES.repairCompass){const target=repairPoints.find(point=>!point.repaired);if(target){const direction=Math.sign(target.x-player.x)||1;ctx.save();ctx.translate(640,78);ctx.fillStyle='#fff0a4';ctx.shadowColor='#ffe159';ctx.shadowBlur=10;ctx.beginPath();ctx.moveTo(direction*14,0);ctx.lineTo(-direction*8,-8);ctx.lineTo(-direction*8,8);ctx.closePath();ctx.fill();ctx.font='800 10px sans-serif';ctx.textAlign='center';ctx.fillText('REPAIR',0,-15);ctx.textAlign='start';ctx.restore();}}
    if(FEATURES.checkpointMapRibbon){const next=checkpoints[world.checkpointIndex+1];if(next){const direction=Math.sign(next.x-player.x)||1;ctx.save();ctx.translate(1145,78);ctx.globalAlpha=.72;ctx.fillStyle='#fff1a4';ctx.shadowColor='#ffe052';ctx.shadowBlur=9;ctx.beginPath();ctx.moveTo(direction*13,0);ctx.lineTo(-direction*8,-7);ctx.lineTo(-direction*8,7);ctx.closePath();ctx.fill();ctx.font='800 10px sans-serif';ctx.textAlign='center';ctx.fillText('NEXT CP',0,-15);ctx.textAlign='start';ctx.restore();}}
    ...
    if((FEATURES.checkpointScaffoldCharge && player.scaffoldCharges>0) || (FEATURES.checkpointGuardSeal && player.checkpointGuard>0)){ctx.save();ctx.fillStyle='#e6fbff';ctx.shadowColor='#77eaff';ctx.shadowBlur=10;ctx.font='800 10px sans-serif';ctx.textAlign='center';const kit=[FEATURES.checkpointScaffoldCharge&&player.scaffoldCharges>0?`CP 設計図 C ×${player.scaffoldCharges}`:'',FEATURES.checkpointGuardSeal&&player.checkpointGuard>0?'守りの印':'' ].filter(Boolean).join('　');ctx.fillText(kit,640,138);ctx.restore();}
    if(FEATURES.recoveryGlow && world.recoveryGlow>0){ctx.save();ctx.globalAlpha=world.recoveryGlow*.45;ctx.fillStyle='#fff19a';ctx.fillRect(0,0,1280,720);ctx.restore();}
    if(FEATURES.repairCombo && world.repairCombo>1){ctx.save();ctx.fillStyle='#fff0a1';ctx.font='800 12px sans-serif';ctx.textAlign='center';ctx.fillText(`REPAIR CHAIN ×${world.repairCombo}`,640,137);ctx.restore();}
