...
const implementations = [
  'systems/stage-builder.js の各ステージ定義へ任意パラメータを追加し、game.js で機能オプションを確認して適用する。',
  'systems/gameplay-options.js の専用フラグと systems/world-rules.js の数値を使い、既存の一時足場・粒子・敵ステートを組み合わせる。',
  '専用画像が必要なため、assets の個別スプライトを追加してから Canvas 描画へ接続する。',
  'セーブ互換性を保つため、snapshot の任意フィールドとして保存し、未設定時は既存挙動に戻す。',
  'コース選択の島ノードに表示するため、systems/stage-builder.js とマップ描画の両方へ状態を渡す。',
